...
public class ConnectionImpl extends AbstractConnection {
    ...
    public ConnectionImpl accept() throws IOException, SocketTimeoutException {
        ...
        returnedConnection.nextSequenceNo = this.nextSequenceNo;
	...
	public void close() throws IOException {
        ...
        Log.writeToLog(fin_pckg, "sending this FIN package" , "ConnectionImpl - close");
        sTimer.run();
        this.state = State.FIN_WAIT_1;
        KtnDatagram rcved_pckg;
        ...
        while(true){
        	after = System.currentTimeMillis();
        	if (after - before > this.TIMEOUT){
        		Log.writeToLog("TIMEOUT while waiting for ACK after sent fin, throwing IOException", "ConnectionImpl - close");
        		throw new IOException("TIMEOUT while waiting for ACK after sent FIN");
        	}
        	rcved_pckg = this.receiveAck();
        	if (rcved_pckg == null){
        		Log.writeToLog("received null pckg while waiting for ACK after sent FIN", "ConnectionImpl - close");
        		continue;
        	}
        	if (rcved_pckg.getFlag() == Flag.ACK && rcved_pckg.getAck() == fin_pckg.getSeq_nr() && rcved_pckg.calculateChecksum() == rcved_pckg.getChecksum()){
        		Log.writeToLog("successfilly delivered FIN package, waiting for FIN from the other side", "ConnectionImpl - close");
        		sTimer.cancel();
        		break;
        	} else {
        		Log.writeToLog("rcved wrong package, ignoring", "ConnectionImpl - close");
        	}    	
        }
        ...
        while (true){
        	after = System.currentTimeMillis();
        	if (after - before > this.TIMEOUT){
        		Log.writeToLog("TIMEOUT while waiting for FIN from the other side", "ConnectionImpl - close");
        		throw new IOException("TIMEOUT while waiting for FIN from the other side");
        	}
        	rcved_pckg = this.receivePacket(true);
        	if (rcved_pckg == null){
        		Log.writeToLog("received null pckg while waiting for FIN from the other side", "ConnectionImpl - close");
        		continue;
        	}
        	if (rcved_pckg.getFlag() == Flag.FIN && rcved_pckg.getChecksum() == rcved_pckg.calculateChecksum()){
        		Log.writeToLog("successfilly acquired FIN package, sending ACK and quitting", "ConnectionImpl - close");
